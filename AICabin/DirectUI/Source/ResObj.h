...
public:
	...
	virtual void SetThemePathFromRDB(LPCTSTR lpszPath);
...
private:
	...
	std::map<tstring, tstring>			m_mapNameTable;			// 保存命名对应表
	...
	std::map<tstring, ImagePathObj*> m_mapImagePathObjTable;		// 保存图片id和路径
	std::map<tstring, tstring>      m_mapStringTable;		// 保存多国语言字符串列表 _RES_TYPE_STRING
