...
ResObj::ResObj(void)
{
	...
	m_rcWindowShadowEdge.top = 12;
	m_rcWindowShadowEdge.bottom = 22;
...
bool ResObj::LoadSkin(xml_document<>* pXmlDoc, LPCTSTR lpszName, UINT uResType)
{
	...
	if (uResType & _RES_TYPE_WINDOW)
	{
		for(pEleSub = pRootEle->first_node("Window"); pEleSub; pEleSub=pEleSub->next_sibling("Window"))
		{
			...
			{
				TemplateIterator AttrIterator = m_mapWindowTable.find(Charset::UTF8ToNative(pszId));
			...
			const char* pszStore =  (attribute = pEleSub->first_attribute("store")) ? attribute->value() :NULL;
			...
			m_mapWindowTable[Charset::UTF8ToNative(pszId)] = pTempObj;
	...
	if (uResType & _RES_TYPE_STYLE)
	{
		...
		if (pEle)
		{
			for(pEleSub=pEle->first_node(); pEleSub; pEleSub=pEleSub->next_sibling())
			{
				...
				{
					TemplateIterator AttrIterator = m_mapStyleTable.find(Charset::UTF8ToNative(pszId));
				...
				m_mapStyleTable[Charset::UTF8ToNative(pszId)] = pTempObj;
	...
	if (uResType & _RES_TYPE_IMAGE)
	{
		...
		if (pEle)
		{
			for(pEleSub=pEle->first_node(); pEleSub; pEleSub=pEleSub->next_sibling())
			{
				...
				// 语言不对不导入
				if (pszLanguage) strLanguage =  Charset::UTF8ToNative(pszLanguage);
				const char* pszPath = (attribute = pEleSub->first_attribute("path")) ? attribute->value() :NULL;
				if (pszPath)
				{
					tstring strImageID = _T("#") + Charset::UTF8ToNative(pszId);
					tstring strImagePath = Charset::UTF8ToNative(pszPath);
				
					ImagePathObj* pImagePathObj = NULL;
					std::map<tstring, ImagePathObj*>::iterator imageIterator = m_mapImagePathObjTable.find(strImageID);
					if (imageIterator != m_mapImagePathObjTable.end())
					{
	...
	if (uResType & _RES_TYPE_FONT)
	{
		...
		if (pEle)
		{
			for(pEleSub=pEle->first_node(); pEleSub; pEleSub=pEleSub->next_sibling())
			{
				...
				tstring strID = Charset::UTF8ToNative(pszId);
				FontObj* pFontObj = GetUIRes()->GetFont(strID.c_str());
				...
				while (pAttr)
				{
					const char* szName = pAttr->name();
					const char* szValue = pAttr->value();
					pFontObj->SetAttribute(Charset::UTF8ToNative(szName).c_str(), Charset::UTF8ToNative(szValue).c_str());
					pAttr = pAttr->next_attribute();
				}
	...
	if (uResType & _RES_TYPE_STRING)
	{
		...
		if (pEle)
		{
			...
			{
				const char* pszId = (attribute = pEleSub->first_attribute("id")) ? attribute->value() :NULL;
				const char* pszText = pEleSub->value();
				if (!pszId || !pszText)
					continue;

				m_mapStringTable[Charset::UTF8ToNative(pszId)] = Charset::UTF8ToNative(pszText);
			}
		}
	}
		...
		if (pEle)
		{
			for(pEleSub=pEle->first_node(); pEleSub; pEleSub=pEleSub->next_sibling())
			{
				...
				while (pAttr)
				{
					const char* szName = pAttr->name();
					const char* szValue = pAttr->value();
					pColorObj->SetAttribute(Charset::UTF8ToNative(szName).c_str(), Charset::UTF8ToNative(szValue).c_str());
					pAttr = pAttr->next_attribute();
				}
		...
		if (pEle)
		{
			for(pEleSub=pEle->first_node(); pEleSub; pEleSub=pEleSub->next_sibling())
			{
				...
				if (!pszId)
					continue;
				tstring strID = Charset::UTF8ToNative(pszId);
				CurveObj* pCurveObj = GetCurveObj(strID.c_str());
				if (!pCurveObj)
				{
				...
				xml_node<>* pEleCtl = pEleSub->first_node();
				while (pEleCtl)
				{
					const char* szTp = (attribute = pEleCtl->first_attribute("tp")) ? attribute->value() :NULL; 
					const char* szSp = (attribute = pEleCtl->first_attribute("sp")) ? attribute->value() :NULL;
					if (!!szTp && !!szSp)
...
void ResObj::InitResDir(xml_document<>* pXmlDoc)
{
	...
	if (pEle)
	{
		for(xml_node<>* pEleSub= pEle->first_node(); pEleSub; pEleSub=pEleSub->next_sibling())
		{
			...
			m_mapNameTable[Charset::UTF8ToNative(pszTag)] = Charset::UTF8ToNative(pszPath);
		}
	}
...
void ResObj::SetThemePathFromRDB(LPCTSTR lpszPath)
{
	m_strThemePath = _T("");
	if (m_pThemeRdbFile)
	{
	...
	if (!PathIsDirectory(lpszPath) && PathFileExists(lpszPath))
	{
		...
		m_pThemeRdbFile->Open(m_strThemePath.c_str());
	}
...
LPCTSTR ResObj::GetI18NStr(LPCTSTR lpszId, bool* bFind)
{
	...
	std::map<tstring, tstring>::iterator it = m_mapStringTable.find(lpszId);
...
TemplateObj* ResObj::GetWindowTemplateObj(LPCTSTR lpszId)
{
	...
	std::map<tstring, tstring>::iterator it1 = m_mapNameTable.find(_T("W.") + tstring(lpszId));
	if (it1 == m_mapNameTable.end())
		return NULL;
	LoadSkin(it1->second.c_str(), lpszId, _RES_TYPE_WINDOW);
...
const MAPAttr* ResObj::GetTemplateObjAttr(LPCTSTR lpszTemplateID)
{
	if (lpszTemplateID != NULL)
	{
		...
		if (!GetCacheXML())
		{
			std::map<tstring, tstring>::iterator it1 = m_mapNameTable.find(_T("S.") + tstring(lpszTemplateID));
			if (it1 == m_mapNameTable.end())
				return NULL;

			LoadSkin(it1->second.c_str(), lpszTemplateID, _RES_TYPE_STYLE);
...
#ifdef _DEBUG
	std::map<tstring, tstring>::iterator it1 = m_mapNameTable.find(_T("S.") + tstring(lpszId));
	if (it1 == m_mapNameTable.end())
		return false;

	LoadSkin(it1->second.c_str(), lpszId,  _RES_TYPE_STYLE);
	TemplateIterator it = m_mapStyleTable.find(lpszId);
	if (it == m_mapStyleTable.end())
		return false;
#else
