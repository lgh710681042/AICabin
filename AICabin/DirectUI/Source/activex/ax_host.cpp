...
namespace util_activex
{
	...
	private:
		LONG ref_;
		util_activex::ScopedComPtr<IOleInPlaceActiveObject> active_object_;
	};
	...
	bool AxHost::SetRect(const RECT& rect)
	{
		if(!::EqualRect(&bounds_, &rect))
		{
			...
			if(windowless_)
			{
				delegate_->OnAxInvalidate(bounds_);
			}
		}
	...
	void AxHost::Draw(HDC hdc, const RECT& rect)
	{
	...
	bool AxHost::OnSetCursor(const POINT& point)
	{
		if(!windowless_)
		{
			return false;
	...
	STDMETHODIMP AxHost::QueryInterface(REFIID iid, void** object)
	{
		...
		*object = NULL;
		...
		{
			*object = static_cast<IObjectSafety*>(this);
		}
		else
		{
			hr = E_NOINTERFACE;
	...
	// IObjectSafety
	STDMETHODIMP AxHost::GetInterfaceSafetyOptions(
		REFIID riid,
		...
		DWORD __RPC_FAR *pdwEnabledOptions)
	{
		...
		*pdwEnabledOptions = 0;
		...
		LPVOID pVoid = NULL;
		this->QueryInterface(IID_IOleClientSite, (LPVOID*)&pVoid);
		if (NULL == pVoid)
		{
			return E_NOINTERFACE;
		}
		OLECHAR szGUID[39];
		StringFromGUID2(riid, szGUID, 39);
	...
	// IObjectSafety
	STDMETHODIMP AxHost::SetInterfaceSafetyOptions(
		REFIID riid,
	...
	bool AxHost::ActivateAx()
	{
		...
		{
			util_activex::ScopedComPtr<IOleClientSite> client_site;
			client_site.QueryFrom(controlling_unknown());
			ole_object_->SetClientSite(client_site);
		}

		util_activex::ScopedComPtr<IPersistStreamInit> stream_init;
		...
		util_activex::ScopedComPtr<IAdviseSink> advise_sink;
		...
		if((misc_status_ & OLEMISC_INVISIBLEATRUNTIME) == 0)
		{
			util_activex::ScopedComPtr<IOleClientSite> client_site;
			client_site.QueryFrom(controlling_unknown());
			ole_object_->DoVerb(OLEIVERB_INPLACEACTIVATE,
				NULL, client_site, 0, delegate_->GetAxHostWindow(), &bounds_);
		}

		util_activex::ScopedComPtr<IObjectWithSite> site;
