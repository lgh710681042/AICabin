...
UINT CWindowUI::DoModal(int nCmdShow /*= SW_SHOW*/)
{
	...
	while(::IsWindow(hWnd) && ::GetMessage(&msg, NULL, 0, 0) ) 
	{
		...
		if (msg.message == WM_CLOSE)
		{
			...
			{
				HWND hCloseWndParent = hWndParent;
				while (hCloseWndParent)
				{
					if (hCloseWndParent == msg.hwnd)
					{
						msg.hwnd = m_hWnd;
						break;
					}

					hCloseWndParent = GetWindow(hCloseWndParent, GW_OWNER);
				}
	...
	else
	{
		if (!m_pShadowWindow)
		{
			if (IsWindow(GetHWND()))
			{
				...
				m_pShadowWindow->ShowWindow(SW_SHOWNOACTIVATE);
			}
		}
	}
}
...
void CWindowUI::SetEventHover(CControlUI* pEventHover)
{
	m_pEventHover = pEventHover;
}
