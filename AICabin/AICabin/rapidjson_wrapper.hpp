...
#pragma region string
	...
	template <typename GenValue>
	void SetStringByPointer(GenValue& rv, const std::basic_string<typename GenValue::Ch>& path, const std::basic_string<typename GenValue::Ch>& val)
	{
		GenericPointer<GenValue::ValueType> point(path);
		if (point.IsValid())
		{
...
#pragma region int
	...
	template <typename GenValue>
	void SetIntByPointer(GenValue& rv, const std::basic_string<typename GenValue::Ch>& path, int val)
	{
		GenericPointer<GenValue::ValueType> point(path);
		if (point.IsValid())
		{
...
#pragma region object
	...
	//只支持读取对象
	template <typename GenValue>
	typename GenValue::ConstObject GetObjectEx(const GenValue& rv)
	{
		if (rv.IsObject())
		{
			return rv.GetObjectEx();
		}
