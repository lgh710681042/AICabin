...
#include "../../utilities/util/include/string_cvt.h"
...
namespace util
{
	...
	public:
		...
		std::string ToString() const//to utf8 string
		{
		...
		const RpJsonValue& GetValue() const
		{
			return *(root_.get());
		}
		...
		{
			return ((RpJsonDoc*)root_.get())->Parse(data).HasParseError();
		}

		static void ReadValue(char*& t, const rapidjson::Value& val)
		{
			if (val.IsString())
				t = (char*)val.GetString();
		}

		static void ReadValue(std::string& t, const rapidjson::Value& val)
		{
			if (val.IsString())
				t = val.GetString();
		}

		static void ReadValue(std::wstring& t, const rapidjson::Value& val)
		{
			if (val.IsString())
				t = util::utf8_to_unicode(val.GetString());
		}

		static void ReadValue(uint8_t& t, const rapidjson::Value& val)
		{
			if (val.IsInt())
				t = (uint8_t)val.GetInt();
		}

		static void ReadValue(int8_t& t, const rapidjson::Value& val)
		{
			if (val.IsInt())
				t = (int8_t)val.GetInt();
		}

		static void ReadValue(int16_t& t, const rapidjson::Value& val)
		{
			if (val.IsInt())
				t = (int16_t)val.GetInt();
		}

		static void ReadValue(uint16_t& t, const rapidjson::Value& val)
		{
			if (val.IsInt())
				t = (uint16_t)val.GetInt();
		}

		static void ReadValue(int& t, const rapidjson::Value& val)
		{
			if (val.IsInt())
				t = val.GetInt();
		}
		...
		static void ReadValue(uint32_t& t, const rapidjson::Value& val)
		{
			if (val.IsUint())
				t = val.GetUint();
		}

		static void ReadValue(int64_t& t, const rapidjson::Value& val)
		{
			if (val.IsInt64())
				t = val.GetInt64();
		}

		static void ReadValue(uint64_t& t, const rapidjson::Value& val)
		{
			if (val.IsUint64())
				t = val.GetUint64();
		}

		static void ReadValue(double& t, const rapidjson::Value& val)
		{
			if (val.IsNumber())
				t = val.GetDouble();
		}

		static void ReadValue(float& t, const rapidjson::Value& val)
		{
			if (val.IsNumber())
				t = static_cast<float>(val.GetDouble());
		}

		static void ReadValue(bool& t, const rapidjson::Value& val)
		{
			if (val.IsBool())
				t = val.GetBool();
		}
	...
	private:
		...
		template<>// wstring
		RpJsonValue to_json_value(const std::wstring& value)
		{
			return std::move(RpJsonValue(util::unicode_to_utf8(value), alloc_));
		}
